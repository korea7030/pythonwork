...
class Game:
    def __init__(self, screen_width, screen_height, show_game=True):
        ...
        # 도로의 크기는 스크린의 반으로 정하며, 도로의 좌측 우측의 여백을 계산
        ...
        # 자동차와 장애물의 초기 위치와, 장애물의 각각의 속도를 정함
    ...
    def _prepare_display(self):
        """게임을 화면에 보여주기 위해 matplotlib 으로 출력할 화면 설정 """
        fig, axis = plt.subplots(figsize=(4,6))
        fig.set_size_inches(4,6)
        # 화면을 닫으면 프로그램을 종료
        fig.canvas.mpl_connect('clost_event', exit)
        ...
        plt.draw()
        # 게임을 진행하며 화면을 업데이트 할 수 있도록 interactive 모드로 설정
        plt.ion()
        plt.show()
    ...
    def _get_state(self):
        ...
        빈 공간인 경우에는 0, 사물이 있는 경우에는 1이 들어있는 1차원 배열
        계산의 편의성을 위해 2차원 -> 1차원으로 변환하여 사용
        """
    ...
    def _draw_screen(self):
        ...
        # 자동차, 장애물들을 1x1 크기의 정사각형으로 그리도록 하며, 좌표를 기준으로 중앙에 위치 시킨다.
        ...
        # 게임의 다음 단계 진행을 위해 matplot 의 이벤트 루프를 잠시 멈춤
        plt.pause(0.001)
    ...
    def _update_car(self, move):
        ...
        자동차 위치 제한을 도로가 아니라 화면의 좌우측 끝으로 하고, 도로를 넘어가면 패널티를 주도록 학습해서 도로를 넘지 않게 만들면 좋을 듯"""

        # 자동차의 위치가 도로의 좌측을 넘지 않도록 한다
        self.car["col"] = max(self.road_left, self.car["col"] + move)
        # 자동차의 위치가 도로의 우측을 넘지 않도록 한다.
    ...
    def _is_gameover(self):
        # 장애물과 자동차가 충돌했는지를 파악.
        # 사각형 박스의 충돌을 체크하는 것이 아니라 좌표를 체크하는 것이어서 화면에는 약간 다르게 보일 수 있음.
        if ((self.car["col"] == self.block[0]["col"] and self.car["row"] == self.block[0]["row"]) or (self.car["col"] == self.block[1]["col"] and self.car["row"] == self.block[1]["row"])):
    ...
    def step(self, action):
        # action: 0: 좌, 1: 유지, 2: 우
        # action - 1 을 하여, 좌표를 액션이 0 일 경우 -1 만큼, 2 일 경우 1 만큼 옮김.
        self._update_car(action - 1)
        # 장애물을 이동시킵니다. 장애물이 자동차에 충돌하지 않고 화면을 모두 지나가면 보상을 얻음.
        escape_reward = self._update_block()
        # 움직임이 적을 경우에도 보상을 줘서 안정적으로 이동하는 것 처럼 보이게 만듦.
        stable_reward = 1. / self.screen_height if action == 1 else 0
        # 게임이 종료됐는지를 판단합니다. 자동차와 장애물이 충돌했는지를 파악함.
        gameover = self._is_gameover()

        if gameover:
            # 장애물에 충돌한 경우 -2점을 보상으로 줍니다. 장애물이 두 개이기 때문
            # 장애물을 회피했을 때 보상을 주지 않고, 충돌한 경우에만 -1점을 주어도 됨.
            reward = -2
        else:
