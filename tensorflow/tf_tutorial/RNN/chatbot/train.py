...
def train(dialog, batch_size = 100, epoch=100):
    print("vocab_size : ", dialog.vocab_size)
    model = Seq2Seq(dialog.vocab_size)

    with tf.Session() as sess:
        # TODO : 세션을 로드하고 로그를 위한 summary 저장 등의 로직을 SeqSeq 모델로 넣을 필요가 있음.
        ckpt = tf.train.get_checkpoint_state(FLAGS.train_dir)

        if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
            print("다음 파일에서 모델을 읽는 중입니다..", ckpt.model_checkpoint_path)
...
def test(dialog, batch_size=100):
    ...
    model = Seq2Seq(dialog.vocab_list)

    with tf.Session() as sess:
        ckpt = tf.train.get_checkpoint_state(FLAGS.train_dir)
        print("다음  파일에서 모델을 읽는 중입니다..", ckpt.model_check_point_path)
