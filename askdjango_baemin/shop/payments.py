...
class BaseOrder(models.Model):
    ...
    is_faild=property(lambda self: self.status == 'failed')
    ...
    def cancel(self, reason=None, commit=True):
        ...
        except Iamport.ResponseError as e:  # 취소시 오류 예외처리(취소된 결제는 에러 발생)
    ...
    def status_html(self):
        ...
        html = '''
            <span style="color: {text_color};" title="this is title">
                ...
                {label}
            </span>'''.format(class_name=cls, text_color=text_color, label=self.get_status_display())
