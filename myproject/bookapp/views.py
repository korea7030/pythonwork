	...
	if request.method == 'POST' :
		...
		if form.is_valid():
			...
			if book_format == 'eBook':
				cate_id = BookCategory.objects.filter(Q(cate_1 ='e북') & Q(cate_2 = request.POST.get('category'))).values()
			else:
				cate_id = BookCategory.objects.filter(~Q(cate_1 ='e북') & Q(cate_2 = request.POST.get('category'))).values()
		...
		if q != '':
			...
			if category != '':
				if SWITCH_MAP[category] == 1:	# 책 카테고리별
					bookList = BookInfo.objects.order_by('-start_date').all().select_related("cate_cd").filter(Q(cate_cd__cate_2 =q))
					# print(bookList[0].cate_cd.cate_2)
				elif SWITCH_MAP[category] == 2: # 별점별
					bookList = BookInfo.objects.order_by('-start_date').all().select_related("cate_cd").filter(Q(rating=int(q)))
				elif SWITCH_MAP[category] == 3: # 책 형태별
					bookList = BookInfo.objects.order_by('-start_date').all().select_related("cate_cd").filter(Q(book_format=q))
				elif SWITCH_MAP[category] == 4:  # 빌린여부
					bookList = BookInfo.objects.order_by('-start_date').all().select_related("cate_cd").filter(Q(borrowed_yn=q))
		else:
...
def book_dash(request):
	...
	# 책 종류별 Count
	book_group_category = BookInfo.objects.all().select_related('cate_cd').values('cate_cd__cate_2').annotate(total=Count('cate_cd__cate_2')).order_by('-total')
	...
	books = BookInfo.objects.order_by('-start_date').values('book_title', 'poster_url').select_related("cate_cd")[0:5]
	# print(books)
...
def ajax_analytic_word(request):
	...
	if request.is_ajax():
		...
		edges_dict = [{"source":node_map[edges[i][0]], "target":node_map[edges[i][1]],"value":weight_list[i]} for i in range(len(edges))] # source : 출발노드 , target: 도착노드, value: edge 거리(상관계수)
...
def get_array(Table, column, cate):
	...
	if type(cate) is str:
		...
		rows = Table.objects.all().select_related('cate_cd').filter(Q(cate_cd__cate_2 = cate[0]['cate_cd__cate_2'])).values(column)
